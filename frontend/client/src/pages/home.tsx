...
const CountUp = ({
  ...
  return (
    <div ref={ref} className="font-mono font-bold text-[var(--color-teal-600)]">
      {count}
      {suffix}
    </div>
  );
  ...
  return (
    <div className="bg-[var(--color-navy-900)] text-white font-sans selection:bg-[var(--color-teal-600)] selection:text-white min-h-screen flex flex-col overflow-x-hidden">
      ...
      <main className="flex-grow pt-32 sm:pt-36 md:pt-40">
        ...
        <section className="pb-24 bg-[var(--color-navy-900)]">
          <div className="container-editorial mb-16 px-6">
            <motion.h1
              ...
              No hidden{" "}
              <span className="text-[var(--color-teal-400)]">clauses.</span>
              <br />
              No surprises.
            </motion.h1>
            <motion.p
              ...
              IndSure analyzes your entire insurance portfolio — unbiased,
        ...
        <section
          id="demo"
          className="scroll-mt-28 py-24 bg-[var(--color-cream-main)] text-[var(--color-text-main)] border-y border-[var(--color-border-light)]">
          <div className="container-editorial">
            <div className="flex flex-col md:flex-row justify-between items-end mb-16 gap-8">
              <div>
                <h2 className="text-4xl md:text-5xl font-serif font-bold mb-4 text-[var(--color-navy-900)]">
                  Forensic Analysis
                </h2>
            ...
            <motion.div
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-12">
                <div className="col-span-1 border-r border-[var(--color-border-light)] pr-8">
                  <span className="text-sm font-mono uppercase tracking-widest text-[var(--color-text-muted)]">
                    Insurance Health Score
                  </span>
                  <div className="text-8xl font-serif font-bold text-[var(--color-teal-600)] mt-4 mb-2">
                    <CountUp target={68} />
                  </div>
                  <p className="text-sm text-[var(--color-teal-600)] font-medium">
                    Top 15% of policyholders
                  </p>
                ...
                <div className="col-span-2 space-y-8">
                  <h3 className="text-2xl font-serif font-bold mb-6 text-[var(--color-navy-900)]">
                    Coverage Breakdown
                  </h3>
                  ...
                  {[
                    {
                      ...
                      color: "bg-[var(--color-teal-600)]",
                    },
                    {
                      label: "Waiting Period Risk",
                      val: 30,
                      status: "3 Years Remaining (High Risk)",
                      color: "bg-red-400",
                    },
                    {
                      ...
                      val: 0,
                      status: "Not Covered",
                      color: "bg-[var(--color-border-medium)]",
                    },
                  ].map((item, i) => (
                    <div key={i}>
                      <div className="flex justify-between mb-2 text-sm font-medium">
                        <span className="text-[var(--color-navy-900)]">
                          {item.label}
                        </span>
                  ...
                  <div className="bg-[var(--color-cream-main)] p-4 rounded-lg border border-[var(--color-border-light)] mt-8 flex items-center gap-4">
                    ...
                    <p className="text-sm text-[var(--color-text-secondary)]">
                      <strong>Recommendation:</strong> Consider porting to a
                      policy with zero room-rent capping to avoid proportionate
                      deduction.
                    </p>
                  </div>
                </div>
        ...
        <section className="py-24 bg-[var(--color-navy-900)] text-white">
          <div className="container-editorial">
            <h2 className="text-4xl md:text-5xl font-serif font-bold mb-16 text-center text-white">
              Why traditional policies fail you
            </h2>

            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              {[
                {
                  title: "Complex by Design",
                  desc: "Policies are intentionally vague to limit claims.",
                },
                {
                  title: "Silent Gaps",
                  desc: "Critical exclusions often go unnoticed until admission.",
                },
                {
                  title: "Costly Overlaps",
                  desc: "You might be paying for duplicate coverage.",
                },
                {
                  title: "Biased Advice",
                  desc: "Agents are incentivized to sell, not to analyze.",
                },
              ].map((card, i) => (
                <motion.div
                  key={i}
                  ...
                  <p className="text-[var(--color-white-muted)] text-sm leading-relaxed group-hover:text-white/90">
                    {card.desc}
                  </p>
                </motion.div>
              ))}
        ...
        <section className="py-32 bg-[var(--color-cream-main)] text-[var(--color-text-main)] border-t border-[var(--color-border-light)] overflow-hidden">
          <div className="container-editorial">
            <div className="flex flex-col md:flex-row justify-between items-end mb-20 scroll-mt-20">
              <h2 className="text-4xl md:text-5xl font-serif font-bold text-[var(--color-navy-900)]">
                From chaos to clarity
                <br />
                in 4 steps.
              </h2>
              <Link href="/policychecker">
                <span className="text-[var(--color-teal-600)] font-medium hover:underline cursor-pointer">
              ...
              {[
                {
                  step: "01",
                  title: "Upload",
                  desc: "Upload your policy PDF securely.",
                },
                {
                  step: "02",
                  title: "Decipher",
                  desc: "AI extracts hidden clauses.",
                },
                {
                  step: "03",
                  title: "Audit",
                  desc: "50+ risks checked instantly.",
                },
                {
                  step: "04",
                  title: "Report",
                  desc: "Get your coverage score.",
                },
              ].map((item, i) => (
                <motion.div
                  key={i}
                  ...
                  <h3 className="text-xl font-bold mb-2 text-[var(--color-navy-900)]">
                    {item.title}
                  </h3>
                  <p className="text-sm text-[var(--color-text-secondary)]">
                    {item.desc}
