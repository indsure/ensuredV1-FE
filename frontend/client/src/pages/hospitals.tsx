...
const CollapsibleSection = ({
  ...
  return (
    <section
      className={`border-t border-[var(--color-border-subtle)] ${className}`}>
      <button
        ...
        {isOpen ? (
          <ChevronUp className={`w-6 h-6 ${titleClassName}`} />
        ) : (
          <ChevronDown className={`w-6 h-6 ${titleClassName}`} />
        )}
      </button>
      <AnimatePresence>
        {isOpen && (
...
export default function HospitalFilter() {
  ...
  const getStats = (data: FilterResult) => {
    const source =
      data.pincodeLevel.length > 0 ? data.pincodeLevel : data.cityLevel;
    const totalHospitals = source.reduce(
      (acc, loc) => acc + loc.unique_hospital_count,
      0,
    );

    const uniqueInsurers = new Set<string>();
    source.forEach((loc) =>
      loc.insurers.forEach((i) => uniqueInsurers.add(i.insurer_slug)),
    );
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${getApiBase()}/api/hospitals/filter?${params.toString()}`,
      );
  ...
  return (
    <div className="bg-[var(--color-navy-900)] text-white font-sans min-h-screen flex flex-col">
      ...
      <main className="flex-grow pt-32 sm:pt-36 md:pt-40 pb-12 md:pb-24 overflow-x-hidden">
            ...
            <div className="container-editorial w-full max-w-4xl">
              <motion.div
                ...
                <h1 className="text-5xl md:text-7xl font-serif font-bold mb-6 text-white leading-tight">
                  Hospital{" "}
                  <span className="text-[var(--color-teal-400)]">Network</span>{" "}
                  Finder
                </h1>
              ...
              <div className="bg-[var(--color-blue-800)] border border-[var(--color-border-subtle)] rounded-2xl p-8 shadow-2xl">
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                  <div className="space-y-2">
                    <Label className="text-[var(--color-white-muted)]">
                      State
                    </Label>
                    <Combobox
        ...
        {/* Loading Skeleton */}
        {loading && !results && <HospitalSkeleton />}
        ...
        {results && !loading && (
          <>
                  ...
                  <div className="grid md:grid-cols-2 gap-8 items-start">
                    <div>
                      ...
                      <p className="text-[var(--color-white-muted)] text-lg">
                        Found{" "}
                        <span className="text-[var(--color-teal-400)] font-bold">
                          {getStats(results).totalHospitals} hospitals
                        </span>{" "}
                        across{" "}
                        <span className="text-white font-bold">
                          {getStats(results).totalInsurers} insurers
                        </span>{" "}
                        in this area.
                      </p>
                    </div>
            ...
            <section
              ref={resultsRef}
              className="bg-[var(--color-cream-main)] text-[var(--color-text-main)] py-12">
              <div className="container-editorial">
                {/* City-Level Results */}
          ...
          <div className="container-editorial">
            <div className="max-w-3xl mx-auto">
              <p className="text-lg text-[var(--color-text-secondary)] mb-4 leading-relaxed text-center">
                Hospital Network Finder shows how well each insurer is networked
                in your area.
              </p>
              <p className="text-lg text-[var(--color-text-secondary)] mb-4 leading-relaxed text-center">
                Instead of scanning PDFs or relying on sales claims, you get a
                direct comparison of hospital counts by insurer at:
              </p>
              ...
              <p className="text-lg text-[var(--color-text-secondary)] leading-relaxed text-center italic">
                This helps you evaluate network strength before buying health
                insurance, not after a claim is rejected.
              </p>
            </div>
          </div>
        </CollapsibleSection>
            ...
            <div className="grid md:grid-cols-2 gap-12 items-center">
              <div>
                ...
                <p className="text-lg text-[var(--color-white-muted)] leading-relaxed font-medium text-white mb-2">
                  We extracted, standardized, and indexed official network lists
                  into a single, queryable dataset.
                </p>
              ...
              <div className="bg-[var(--color-blue-800)] border border-[var(--color-border-subtle)] rounded-xl p-8">
                <h3 className="text-white font-serif text-xl mb-6 border-b border-[var(--color-border-subtle)] pb-2">
                  Coverage Snapshot
                </h3>
                  ...
                  <div className="flex items-start gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-[var(--color-white-muted)]">
                        Verified from official insurer network lists
                      </p>
                    </div>
                  </div>
                  <div className="flex items-start gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-[var(--color-white-muted)]">
                        Nationwide urban and semi-urban coverage
                      </p>
                    </div>
                  </div>
                  <div className="flex items-start gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-[var(--color-white-muted)]">
                        Granular, locality-level insight
                      </p>
                    </div>
                  </div>
                  <div className="flex items-start gap-4">
          ...
          <div className="container-editorial">
            ...
            <div className="max-w-3xl mx-auto bg-[var(--color-blue-50)] border border-[var(--color-blue-100)] rounded-lg p-6 flex flex-col md:flex-row items-center gap-4 text-center md:text-left">
              ...
              <p className="text-[var(--color-navy-900)] text-sm leading-relaxed">
                ...
                These network counts are based on official insurer lists
                processed by our engine. While we aim for accuracy, hospital
                tie-ups change frequently. Always verify with the insurer or TPA
                before planned hospitalization. This tool tracks network{" "}
                <em>presence</em>, not quality of care.
              </p>
            </div>
          </div>
        </section>
