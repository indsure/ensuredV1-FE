      ...
      <div className="card-white p-5 sm:p-6 md:p-8 lg:p-12 relative overflow-hidden shadow-lg border-t-4 border-[var(--color-teal-600)]">
        ...
        <div className="mb-8 sm:mb-12 border-b border-[var(--color-border-light)] pb-8 sm:pb-12">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
            <div className="space-y-2">
              <label className="text-xs uppercase tracking-widest text-[var(--color-navy-900)] font-semibold">
                State
              </label>
          ...
          {tier && (
            <p className="mt-4 text-sm text-[var(--color-teal-600)] flex items-center gap-2 flex-wrap">
              ...
              Analysis will be calibrated for{" "}
              <strong>{getTierDescription(tier)}</strong>.
            </p>
          )}
        </div>
        ...
        <div>
          ...
          <div
            ...
            <p className="text-[var(--color-navy-900)] font-medium mb-1 text-base sm:text-lg">
              Upload Policy PDFs
            </p>
          ...
          <div className="flex items-center justify-center gap-3 sm:gap-4 mt-6 sm:mt-8">
            ...
            <span className="text-xs text-[var(--color-text-muted)] uppercase tracking-widest shrink-0">
              OR
            </span>
            <div className="h-[1px] bg-[var(--color-border-light)] flex-grow min-w-0"></div>
          </div>
          ...
          <div className="mt-6 sm:mt-8 text-center">
            <Button
              ...
              variant="outline"
              className="w-full sm:w-auto text-[var(--color-teal-600)] border-[var(--color-teal-600)] hover:bg-[var(--color-teal-600)] hover:text-white h-11 sm:h-12 px-4 sm:px-6 text-sm sm:text-base">
    ...
    <div className="w-full max-w-6xl mx-auto px-0 sm:px-2">
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-end gap-4 mb-6 sm:mb-8 border-b border-[var(--color-border-light)] pb-4 sm:pb-6">
        <div>
          ...
          <h2 className="text-2xl sm:text-3xl md:text-4xl font-serif text-[var(--color-navy-900)]">
            Head-to-Head
          </h2>
        </div>
        <Button
          variant="ghost"
          onClick={onReset}
          className="self-start sm:self-auto text-[var(--color-text-secondary)] hover:text-[var(--color-teal-600)] text-sm sm:text-base -ml-2 sm:ml-0">
          ← Start Over
        </Button>
      </div>
        ...
        <table className="w-full min-w-[640px] text-left border-collapse">
          <thead>
            <tr className="border-b border-[var(--color-border-light)]">
              ...
              {names.map((name: string, i: number) => (
                <th
                  key={i}
                  className="p-3 sm:p-4 md:p-6 min-w-[120px] w-1/4 font-serif text-sm sm:text-base md:text-xl text-[var(--color-navy-900)] bg-[var(--color-cream-dark)] border-l border-[var(--color-border-light)] align-bottom pb-4 md:pb-8">
                  {name}
                </th>
              ))}
            </tr>
          ...
          <tbody className="divide-y divide-[var(--color-border-light)]">
            {data.map((row: any, i: number) => (
              <tr
                key={i}
                className="group hover:bg-[var(--color-cream-main)] transition-colors">
      ...
      <div className="mt-8 sm:mt-12 p-4 sm:p-6 md:p-8 border-l-4 border-[var(--color-gold-500)] bg-white shadow-md rounded-r-lg">
        <h4 className="font-serif text-lg sm:text-xl text-[var(--color-gold-500)] mb-2">
          Sach AI Verdict
        </h4>
        <p className="text-[var(--color-text-secondary)] leading-relaxed text-sm sm:text-base md:text-lg">
          <strong>{names[1]}</strong> offers the best value for money regarding
          waiting periods (1 Year for PED), but <strong>{names[0]}</strong>{" "}
          provides better room rent flexibility ("No Cap"). Given your location
          in a Tier 1 city, prioritize "No Cap" on Room Rent to avoid
...
function roomRentLabel(rr: any): {
  value: string;
  sub: string;
  status: "good" | "bad" | "neutral";
} {
  if (!rr) return { value: "—", sub: "Unknown", status: "neutral" };
  if (rr.limit_type === "none")
    return { value: "No Cap", sub: "Any room type", status: "good" };
  if (rr.limit_type === "room_category")
    return {
      value: rr.limit_value || "Capped",
      sub: "Room category limit",
      status: "bad",
    };
  if (rr.limit_type === "specific_amount")
    return {
      value: rr.limit_value || "Capped",
      sub: "Per day limit",
      status: "bad",
    };
  if (rr.limit_type === "percentage_of_si")
    return {
      value: rr.limit_value || "% of SI",
      sub: "% of sum insured",
      status: "neutral",
    };
  return {
    value: rr.limit_value || "—",
    sub: rr.limit_type || "Unknown",
    status: "neutral",
  };
}

function coPayLabel(cp: any): {
  value: string;
  sub: string;
  status: "good" | "bad" | "neutral";
} {
  if (!cp) return { value: "—", sub: "Unknown", status: "neutral" };
  if (!cp.exists || cp.percentage === 0)
    return { value: "None", sub: "Full claim paid", status: "good" };
  return {
    value: `${cp.percentage}%`,
    sub: "You pay this share",
    status: "bad",
  };
}

function waitLabel(wp: any): {
  value: string;
  sub: string;
  status: "good" | "bad" | "neutral";
} {
  ...
  const status: "good" | "bad" | "neutral" =
    months <= 12 ? "good" : months <= 24 ? "neutral" : "bad";
  return {
    value: `${months} months`,
    sub: months <= 12 ? "Excellent" : months <= 24 ? "Average" : "Long wait",
    status,
  };
}

function ncbLabel(be: any): {
  value: string;
  sub: string;
  status: "good" | "bad" | "neutral";
} {
  ...
  if (!ncb?.exists)
    return { value: "None", sub: "No NCB benefit", status: "bad" };
  ...
  return [
    ...
    },
    {
      ...
      values: policies.map((p) => {
        ...
        return {
          value: fmt(si),
          sub: total && total !== si ? `Total: ${fmt(total)}` : "Base cover",
    ...
    },
    {
      feature: "Room Rent",
      values: policies.map((p) =>
        roomRentLabel(d(p).claim_risk_analysis?.room_rent),
      ),
    },
    {
      feature: "Co-Payment",
      values: policies.map((p) =>
        coPayLabel(d(p).claim_risk_analysis?.co_payment),
      ),
    },
    {
    ...
    },
    {
      ...
      values: policies.map((p) => {
        ...
        if (!sl)
          return { value: "—", sub: "Unknown", status: "neutral" as const };
        if (!sl.exists)
          return {
            value: "None",
            sub: "No sub-limits",
        ...
        const status: "good" | "bad" | "neutral" =
          sl.risk_level === "high" || sl.risk_level === "critical"
            ? "bad"
            : "neutral";
        return {
          value: sl.risk_level?.toUpperCase() || "Exists",
          sub: sl.remarks || "Sub-limits apply",
          status,
        };
      }),
    },
    {
      ...
      values: policies.map((p) => {
        ...
        const label = v?.label || "—";
        const status: "good" | "bad" | "neutral" =
          label === "SAFE" ? "good" : label === "RISKY" ? "bad" : "neutral";
        return {
          value: label,
          sub:
            v?.summary?.slice(0, 60) +
              (v?.summary && v.summary.length > 60 ? "…" : "") || "—",
          status,
        };
      }),
    },
  ...
  return (
    <div ref={wrapperRef} className="relative">
      ...
      <button
        ...
        <ChevronDown
          className={`w-4 h-4 text-[var(--color-text-muted)] flex-shrink-0 transition-transform ${open ? "rotate-180" : ""}`}
        />
      </button>
            ...
            {filtered.length === 0 ? (
              ...
              filtered.map((item) => (
                <button
                  ...
                  className={`w-full text-left px-4 py-2.5 text-sm transition-colors cursor-pointer hover:bg-[var(--color-cream-main)] ${
                    item === value
...
function Cell({
  ...
  data: { value: string; sub: string; status: string };
}) {
  const color =
    data.status === "good"
      ? "text-[var(--color-teal-600)] font-semibold"
      : data.status === "bad"
      ...
      <div
        className={`w-9 h-9 rounded-full flex items-center justify-center flex-shrink-0 ${
          file
            ...
            : "bg-white border border-[var(--color-border-medium)] text-[var(--color-text-muted)]"
        }`}>
        {file ? (
      ...
      <div className="flex-grow min-w-0">
        {file ? (
          ...
          <p className="text-sm text-[var(--color-text-muted)]">
            Policy {index + 1} — drop or click to add
          </p>
        )}
      </div>
      {file && (
        <button
          onClick={onRemove}
          className="w-6 h-6 rounded-full bg-white border border-[var(--color-border-medium)] flex items-center justify-center text-[var(--color-text-muted)] hover:text-red-500 hover:border-red-300 transition-colors flex-shrink-0">
          <X className="w-3 h-3" />
        </button>
      )}
    </div>
    ...
    try {
      const results = await Promise.all(
        filledSlots.map(async (file) => {
          ...
          const res = await fetch(`${getApiBase()}/api/extract-policy`, {
            method: "POST",
            body: formData,
            credentials: "include",
          });
  ...
  return (
    <div className="min-h-screen bg-[var(--color-cream-main)] font-sans text-[var(--color-text-main)] flex flex-col">
          ...
          {!policies && (
            <motion.div
              ...
              <div className="text-center mb-10 sm:mb-16 px-2">
                ...
                <h1 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-serif mb-4 sm:mb-6 tracking-tight text-[var(--color-navy-900)]">
                  Compare Policies
                </h1>
                <p className="text-base sm:text-lg md:text-xl text-[var(--color-text-secondary)] max-w-2xl mx-auto font-light leading-relaxed">
                  An unbiased, line-by-line audit of coverage, limitations, and
                  hidden clauses. See what they aren't telling you.
                </p>
              </div>
              ...
              <div className="max-w-2xl mx-auto space-y-8">
                ...
                <div className="card-white p-8 shadow-lg border-t-4 border-[var(--color-teal-600)] relative z-20">
                  <h2 className="font-serif text-xl text-[var(--color-navy-900)] mb-5 flex items-center gap-2">
                    ...
                    Your Location{" "}
                    <span className="text-sm font-sans font-normal text-[var(--color-text-muted)] ml-1">
                  ...
                  {cityTier && (
                    <p className="mt-3 text-sm text-[var(--color-teal-600)] flex items-center gap-1.5">
                      <CheckCircle2 className="w-4 h-4" />
                      Calibrated for{" "}
                      <strong>{getTierDescription(cityTier)}</strong>
                    </p>
                  )}
                </div>
                ...
                <div
                  {...getRootProps()}
                  className={`card-white p-8 shadow-lg border-2 transition-all relative z-10 ${
                    isDragActive
                      ? "border-[var(--color-teal-600)] bg-[var(--color-teal-600)]/5"
                  ...
                  <p className="mt-4 text-xs text-center text-[var(--color-text-muted)]">
                    Drag & drop PDFs anywhere on this card, or click an empty
                    slot to browse
                  </p>
                </div>
                ...
                <Button
                  ...
                  {isExtracting ? (
                    <span className="flex items-center gap-2">
                      ...
                      Extracting {filledSlots.length}{" "}
                      {filledSlots.length === 1 ? "policy" : "policies"}…
                    </span>
                  ) : (
                    <span className="flex items-center gap-2">
                      <Plus className="w-5 h-5" />
                      Compare{" "}
                      {filledSlots.length >= 2 ? filledSlots.length : ""}{" "}
                      Policies
                    </span>
                  )}
                </Button>
                  ...
                  <div className="mt-12">
                    ...
                    <div className="grid md:grid-cols-3 gap-6">
                      <div className="text-center space-y-2 p-4">
                        ...
                        <h4 className="font-serif text-lg text-[var(--color-navy-900)]">
                          Spot Hidden Limits
                        </h4>
                        <p className="text-sm text-[var(--color-text-secondary)]">
                          We fetch the actual policy wordings to expose room
                          rent caps and sub-limits that marketing covers up.
                        </p>
                      </div>
                      <div className="text-center space-y-2 p-4 border-t md:border-t-0 md:border-l border-[var(--color-border-light)]">
                        ...
                        <p className="text-sm text-[var(--color-text-secondary)]">
                          Our deterministic engine scores policies on a
                          100-point scale based on financial risk and coverage
                          reality.
                        </p>
                      </div>
                      <div className="text-center space-y-2 p-4 border-t md:border-t-0 md:border-l border-[var(--color-border-light)]">
                        ...
                        <p className="text-sm text-[var(--color-text-secondary)]">
                          We simulate actual historical claim data from your
                          city to ensure the sum insured isn't an illusion.
                        </p>
                      </div>
                    </div>
                  </div>
              ...
              <div className="max-w-6xl mx-auto">
                ...
                <div className="flex justify-between items-end mb-8 border-b border-[var(--color-border-light)] pb-6">
                  <div>
                    ...
                    <h2 className="text-4xl font-serif text-[var(--color-navy-900)]">
                      Head-to-Head
                    </h2>
                  ...
                  <Button
                    variant="ghost"
                    onClick={handleReset}
                    className="text-[var(--color-text-secondary)] hover:text-[var(--color-teal-600)]">
                    ← Compare Again
                  </Button>
                </div>
                  ...
                  <table className="w-full text-left border-collapse">
                    <thead>
                      <tr className="border-b border-[var(--color-border-light)]">
                        ...
                        {policies.map((p, i) => {
                          const name =
                            p.extracted_data?.policy_metadata
                          ...
                          return (
                            <th
                              key={i}
                              className="p-5 font-serif text-lg text-[var(--color-navy-900)] bg-[var(--color-cream-dark)] border-l border-[var(--color-border-light)] align-bottom pb-6">
                              <div>{name}</div>
                              {insurer && name !== insurer && (
                                <div className="text-xs font-sans font-normal text-[var(--color-text-muted)] mt-1">
                                  {insurer}
                                </div>
                              )}
                            </th>
                ...
                <div
                  ...
                  style={{
                    gridTemplateColumns: `repeat(${policies.length}, 1fr)`,
                  }}>
                  {policies.map((p, i) => {
                    const v = p.extracted_data?.final_verdict;
                    const name =
                      p.extracted_data?.policy_metadata?.plan_name_normalized ||
                      `Policy ${i + 1}`;
                    const label = v?.label || "—";
                    const borderColor =
                      label === "SAFE"
                    ...
                    return (
                      <div
                        key={i}
                        className={`p-6 border-l-4 bg-white shadow-md rounded-r-lg ${borderColor}`}>
                        <div className="flex items-center gap-2 mb-2">
                          <span
                            className={`text-xs font-mono uppercase tracking-widest font-bold ${labelColor}`}>
                            {label}
                          </span>
                        ...
                        <p className="text-sm text-[var(--color-text-secondary)] leading-relaxed">
                          {v?.summary || "No verdict available."}
                        </p>
                        {v?.key_failure_points &&
                          v.key_failure_points.length > 0 && (
      ...
      <Dialog open={showAuthModal} onOpenChange={setShowAuthModal}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle className="font-serif text-[var(--color-navy-900)]">
              Log in to Compare
            </DialogTitle>
            <DialogDescription>
              You need an account to use the policy comparison tool. Log in or
