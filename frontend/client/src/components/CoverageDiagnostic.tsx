...
const STRESS_TEST_SCENARIOS = [
    { name: "diag.s_accident", cost: 100000 },
    { name: "diag.s_knee", cost: 500000 },
    { name: "diag.s_diabetes", cost: 700000 },
    { name: "diag.s_kidney", cost: 1000000 },
    { name: "diag.s_bp", cost: 1300000 },
    { name: "diag.s_liver", cost: 1500000 },
    { name: "diag.s_tb", cost: 2000000 },
    { name: "diag.s_covid", cost: 2500000 },
    { name: "diag.s_heart", cost: 4000000 },
    { name: "diag.s_cancer", cost: 5000000 }
];
...
export function CoverageDiagnostic({ report, effectiveCoverage }: CoverageDiagnosticProps) {
    ...
    const getStatus = (ratio: number) => {
        if (ratio >= 1.0) return { label: t("diag.sufficient"), color: "text-green-700 bg-green-50 border-green-200", barColor: "bg-green-500" };
        if (ratio >= 0.70) return { label: t("diag.borderline"), color: "text-amber-700 bg-amber-50 border-amber-200", barColor: "bg-amber-500" };
        if (ratio >= 0.40) return { label: t("diag.insufficient"), color: "text-orange-700 bg-orange-50 border-orange-200", barColor: "bg-orange-500" };
        return { label: t("diag.severe"), color: "text-red-700 bg-red-50 border-red-200", barColor: "bg-red-600" };
    };
            ...
            <Sheet>
                <SheetTrigger asChild>
                    <button className="text-[13px] text-slate-500 hover:text-slate-800 transition-colors font-normal text-left leading-tight group">
                        {t("diag.q")}<br />
                        <span className="underline decoration-slate-300 underline-offset-2 group-hover:decoration-slate-500">{t("diag.check")}</span> <span className="font-light inline-block ml-0.5 transition-transform group-hover:translate-x-0.5">&rarr;</span>
                    </button>
                </SheetTrigger>
                <SheetContent side="right" className="flex flex-col h-full overflow-hidden w-full sm:w-[450px] sm:max-w-md p-0 border-l border-[var(--color-border-medium)] shadow-xl">
                    ...
                    <div className="flex-none p-6 bg-[var(--color-cream-main)] border-b border-[var(--color-border-medium)] z-10">
                        ...
                        <div className="bg-white px-3 py-2 rounded border border-[var(--color-border-light)] text-sm font-semibold text-[var(--color-navy-900)] inline-flex items-center gap-2 shadow-sm">
                            ...
                            {t("diag.selected", { amount: formatINR(effectiveCoverage) })}
                        </div>
                    </div>
                                ...
                                return (
                                    <div key={scenario.name} className="relative">
                                        <div className="flex justify-between items-end mb-2">
                                            <div>
                                                <div className="font-bold text-sm text-[var(--color-navy-900)]">{t(scenario.name)}</div>
                                                <div className="text-xs text-[var(--color-text-secondary)] uppercase tracking-wider">{t("diag.expected")}</div>
                                            </div>
                                        ...
                                        <div className="flex justify-between items-center">
                                            <div className="flex items-center gap-2">
                                                ...
                                                <span className="text-xs font-mono font-medium text-[var(--color-text-secondary)]">
                                                    {t("diag.pct_covered", { n: percentage })}
                                                </span>
                                            </div>
                    ...
                    <div className="flex-none p-6 bg-[var(--color-navy-900)] text-white border-t border-[var(--color-navy-800)] z-10 shadow-[0_-4px_12px_rgba(0,0,0,0.15)]">
                        {highestRisk ? (
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <div className="font-bold text-lg text-white">{t("diag.comprehensive")}</div>
                                    <div className="text-sm text-green-400">{t("diag.all_ten")}</div>
                                </div>
                            </div>
                        )}
                    </div>
