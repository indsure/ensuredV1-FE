          ...
          {para.split(/(\*\*[\s\S]*?\*\*|`[\s\S]*?`)/g).map((part, j) => {
            if (part.startsWith("**") && part.endsWith("**")) {
              return (
                <strong
                  key={j}
                  className="font-bold text-[var(--color-text-main)]">
                  {part.slice(2, -2)}
                </strong>
              );
            }
            if (part.startsWith("`") && part.endsWith("`")) {
              return (
                <code
                  key={j}
                  className="bg-[var(--color-border-light)] px-1.5 py-0.5 rounded-sm font-mono text-xs text-[var(--color-text-secondary)]">
                  {part.slice(1, -1)}
                </code>
              );
            }
            if (part.trim().startsWith("- ")) {
              ...
              if (listItems.length > 0) {
                return (
                  <ul
                    key={j}
                    className="list-disc pl-5 space-y-1.5 mt-2 mb-2 marker:text-[var(--color-text-muted)]">
                    {listItems.map((item, k) => (
                      <li
                        key={k}
                        className="pl-1 text-[var(--color-text-secondary)]">
                        {item.replace(/^- /, "")}
                      </li>
                    ))}
                  </ul>
...
export default function SachAIChat() {
  ...
  async function sendMessage(text?: string) {
    ...
    try {
      ...
      setMessages((prev: Message[]) => [
        ...prev,
        { role: "assistant", content: "Sach AI is currently unavailable." },
      ]);
    } finally {
      setLoading(false);
      setIsTyping(false);
        ...
        <div className="fixed bottom-6 right-6 w-[95vw] md:w-[400px] h-[600px] max-h-[85vh] bg-white border border-[var(--color-border-main)] shadow-2xl z-50 rounded-xl flex flex-col overflow-hidden animate-in slide-in-from-bottom-5 fade-in duration-300 font-sans">
          ...
          <div className="bg-[var(--color-green-primary)] p-4 flex items-center justify-between shrink-0">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-white font-serif font-bold text-lg leading-none tracking-wide">
                  Sach AI
                </h3>
          ...
          <div className="flex-1 overflow-y-auto p-5 space-y-6 bg-[var(--color-cream-main)]">
            {messages.length === 0 && (
              <div className="h-full flex flex-col items-center justify-center text-center p-4 opacity-60">
                ...
                <p className="font-serif text-[var(--color-text-main)] text-xl mb-2 font-bold">
                  How can I help?
                </p>
