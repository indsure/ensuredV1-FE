/**
 * Forensic Policy Audit Schema (frontend copy for standalone build)
...
export interface ForensicAuditReport {
    ...
    recommendations: {
        ...
        medium_priority: Array<{ action: string; reason: string }>;
        low_priority: Array<{ action: string; reason: string }>;
    };
...
export const isValidVerdict = (label: string): label is ForensicAuditReport['final_verdict']['label'] =>
    ['SAFE', 'BORDERLINE', 'RISKY'].includes(label);
...
export const validateForensicAuditReport = (data: any): data is ForensicAuditReport => {
    try {
        if (!data?.identity || !data?.policy_timeline || !data?.coverage_structure || !data?.final_verdict || !data?.audit_score) return false;
        if (!isValidZone(data.identity.assumed_zone)) return false;
        if (!isValidVerdict(data.final_verdict.label)) return false;
        if (typeof data.audit_score.score !== 'number' || data.audit_score.score < 0 || data.audit_score.score > 100) return false;
        return true;
    } catch {
        return false;
    }
...
export const calculateEffectiveCoverage = (report: ForensicAuditReport): number | null => {
    const base = typeof report.coverage_structure.base_sum_insured === 'number' ? report.coverage_structure.base_sum_insured : null;
    const topUp = report.coverage_structure.top_up.exists && report.coverage_structure.top_up.sum_insured ? report.coverage_structure.top_up.sum_insured : 0;
    const superTopUp = report.coverage_structure.super_top_up.exists && report.coverage_structure.super_top_up.sum_insured ? report.coverage_structure.super_top_up.sum_insured : 0;
