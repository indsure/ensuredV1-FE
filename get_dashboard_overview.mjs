...
async function getOverview() {
  ...
  for (const agent of agents) {
    ...
    if (errs.length > 0) {
      ...
      errs.forEach(c => {
         console.log(`  - ${c.policyholder_name || 'Unnamed Client'}: Error processing policy. Needs manual retry.`);
      });
    }
    ...
    if (done.length > 0) {
      console.log('Outputs Produced:');
         ...
         let flawsCount = 0;
         try {
           if (typeof c.flaws === 'string') flawsCount = JSON.parse(c.flaws).length;
