...
export const CALCULATOR_CONFIG = {
    ...
    // Calibration 9 — New premium estimation (UIIC interpolation lookup)
    premiumDirectLookupZoneB: {
        "<30": {
            500000: 7237,
            1000000: 9407,
            1500000: 10583,
            2000000: 11378,
            2500000: 12500,  // interpolated
            3000000: 13500,  // interpolated
        },
        "30-39": {
            500000: 9777,   // avg of 31-35 (8684) and 36-40 (10553) midpoints
            1000000: 12504,  // avg of 11289 and 13719
            1500000: 14067,  // avg of 12700 and 15434
            2000000: 15122,  // avg of 13653 and 16592
            2500000: 16500,  // interpolated
            3000000: 18000,  // interpolated
        },
        "40-49": {
            500000: 16558,  // avg of 41-45 (15076) and 46-50 (18039)
            1000000: 21525,  // avg of 19599 and 23450
            1500000: 24215,  // avg of 22048 and 26381
            2000000: 26032,  // avg of 23703 and 28360
            2500000: 28000,  // interpolated
            3000000: 30500,  // interpolated
        },
        "50-59": {
            500000: 26364,  // avg of 51-55 (21646) and 56-60 (31081)
            1000000: 34273,  // avg of 28139 and 40406
            1500000: 38557,  // avg of 31657 and 45457
            2000000: 41449,  // avg of 34032 and 48866
            2500000: 45000,  // interpolated
            3000000: 49000,  // interpolated
        },
        "60-65": {
            500000: 36000,  // estimated (UIIC 61-65 not published separately for all SI)
            1000000: 53560,
            1500000: 60000,  // interpolated
            2000000: 65000,  // interpolated
            2500000: 71000,  // interpolated
            3000000: 78000,  // interpolated
        },
    } as Record<PremiumAgeBand, Record<number, number>>,

    premiumZoneMultipliers: {
        "Zone A": 1.17, // metros (Delhi-NCR, Mumbai+region, etc.) ~17% higher than B
        "Zone B": 1.00, // baseline large cities (Bengaluru, Chennai, Kolkata, Pune, etc.)
        "Zone C": 0.93, // rest-of-India band ~7% cheaper than B, matching UIIC C/B
        "Zone D": 0.85, // extra-low band ~15% cheaper than B (within 10–20% range)
    } as Record<ZoneLabel, number>,
};

// ─── Premium Types and Helpers ────────────────────────────────────────────────

type PremiumAgeBand = "<30" | "30-39" | "40-49" | "50-59" | "60-65";
type ZoneLabel = "Zone A" | "Zone B" | "Zone C" | "Zone D";

function getAgeBandForPremium(age: number): PremiumAgeBand {
    if (age < 30) return "<30";
...
// Map your existing CityTier (or similar) into Zone A/B/C/D for premium purposes
function mapCityTierToZone(cityTier: CityTier): ZoneLabel {
    switch (cityTier) {
        case "Metro":
            return "Zone A";
        case "Tier-1":
        ...
        case "Tier-2":
            return "Zone C";
        default:
            return "Zone C"; // Very low-cost areas can be grouped if needed
    }
}

function getDirectBasePremium(age: number, zone: ZoneLabel, si: number): number {
    const band = getAgeBandForPremium(age);
    const table = CALCULATOR_CONFIG.premiumDirectLookupZoneB[band];
    const multiplier = CALCULATOR_CONFIG.premiumZoneMultipliers[zone] ?? 1.0;

    const siKeys = Object.keys(table)
        .map(Number)
        .sort((a, b) => a - b);

    // Exact match
    if (table[si] !== undefined) return table[si] * multiplier;

    // Below minimum — floor to lowest key
    if (si <= siKeys[0]) return table[siKeys[0]] * multiplier;

    // Above maximum — cap at highest key (conservative)
    if (si >= siKeys[siKeys.length - 1]) return table[siKeys[siKeys.length - 1]] * multiplier;

    // Linear interpolation between two nearest SI brackets
    const lower = siKeys.filter((k) => k < si).at(-1)!;
    const upper = siKeys.find((k) => k > si)!;
    const ratio = (si - lower) / (upper - lower);
    const interpolated = table[lower] + ratio * (table[upper] - table[lower]);
...
function estimatePremium(
    pricingAge: number,
    zone: ZoneLabel,
    si: number,                    // full SI in rupees, e.g. 2000000
    familySize: number,
    isFloater: boolean,
    hasSeriousPED: boolean
): EngineResult["premiumEstimate"] {

    // 1. Direct UIIC-anchored base for this age + zone + SI
    const basePremium = getDirectBasePremium(pricingAge, zone, si);

    // 2. Family floater factor (discount for shared pool)
    const familyFactor = !isFloater
        ? 1.0
        : familySize >= 5
            ? 0.80
            : 0.85;
    ...
    // 3. PED loading
    const pedFactor = hasSeriousPED ? 1.25 : 1.0;

    // 4. Annual estimate (UIIC is a point estimate; window ±20% for plan variance)
    const annualMid = basePremium * familyFactor * pedFactor;
    const annualMin = Math.round(annualMid * 0.85);  // cheaper/bare plans
    const annualMax = Math.round(annualMid * 1.20);  // richer/comprehensive plans
...
export function calculateHealthCover(inputs: UserInputs): EngineResult {
    ...
    let pricingAge = age;
    if (inputs.familyStructure === "Parents included") {
        pricingAge = Math.max(pricingAge, inputs.fatherAge || 0, inputs.motherAge || 0);
    } else if (inputs.familyStructure === "Couple" || inputs.familyStructure === "Couple + kids") {
        pricingAge = Math.max(pricingAge, inputs.spouseAge || 0);
    }

    const zoneLabel: ZoneLabel = mapCityTierToZone(inputs.cityTier);

    let familySize = 1;
    if (inputs.familyStructure === "Couple") familySize = 2;
    if (inputs.familyStructure === "Couple + kids") familySize = 2 + (inputs.childCount || 1);
    if (inputs.familyStructure === "Parents included") {
        familySize = 1 + (inputs.spouseAge ? 1 : 0) + (inputs.childCount || 0) + (inputs.fatherAge ? 1 : 0) + (inputs.motherAge ? 1 : 0);
        if (familySize < 2) familySize = 2;
    }
    ...
    const hasSeriousPED =
        ((inputs.preExistingConditions?.length ?? 0) > 0 && !inputs.preExistingConditions?.includes("none")) ||
        inputs.recurringExpenses === "Chronic but stable";
